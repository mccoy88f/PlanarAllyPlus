...
RUN mkdir -p /planarally/data /planarally/static/assets /planarally/static/mods /planarally/static/temp/dungeons /planarally/extensions && chown -R 9000:9000 /planarally
...
RUN apt-get update && apt-get install --no-install-recommends dumb-init curl libffi-dev libssl-dev gcc \
    libegl1 libgl1 libgles2-mesa -y && \
