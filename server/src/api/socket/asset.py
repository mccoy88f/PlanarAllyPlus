...
async def set_asset_options(sid: str, raw_data: Any):
    ...
    entry = AssetEntry.get_or_none(id=asset_options.asset)
    if entry is None:
        logger.error(f"Attempt to set options for unknown asset by {auth.get_authorized_user(sid).name}")
        return

    entry.options = asset_options.options
    entry.save()
