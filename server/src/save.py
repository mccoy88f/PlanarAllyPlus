...
def upgrade(
    ...
    if version == 94:
        ...
        # No-op to align with previous SAVE_VERSION 122 if necessary
        pass
    elif version == 123:
        # Upstream: Initiative update (effects timing)
        with db.atomic():
        ...
        # Upstream: Add AssetRectVariant & Migrate/Remove old variant setup
        with db.atomic():
            db.execute_sql(
        ...
        # Upstream (PA dev): ambient light + flood-fill aura — version number shifted (PA+ uses 117–126 elsewhere)
        with db.atomic():
        ...
        # AI Generator: Compendium translation source/target language (PA UI locales)
        with db.atomic():
            column_exists = False
            cursor = db.execute_sql("PRAGMA table_info(user_options)")
            for row in cursor.fetchall():
                if row[1] == "openrouter_compendium_translate_source":
                    column_exists = True
                    break
            if not column_exists:
                db.execute_sql(
                    "ALTER TABLE user_options ADD COLUMN openrouter_compendium_translate_source TEXT DEFAULT 'auto'"
                )
                db.execute_sql(
